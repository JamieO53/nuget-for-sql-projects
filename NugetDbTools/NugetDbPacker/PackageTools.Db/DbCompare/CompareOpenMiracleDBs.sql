...
from    [DBOpenMiracle].sys.objects t
join    [DBOpenMiracle].sys.schemas s
    on  s.schema_id = t.schema_id
left join [DBOpenMiracle].sys.dm_db_partition_stats p
...
select  s.name + '.' + t.name $(db2)Object,
        ...
        coalesce(sum(p.row_count), 0) $(db1)Count
from    [OpenMiracle.Cleanup1].sys.objects t
join    [OpenMiracle.Cleanup1].sys.schemas s
    on  s.schema_id = t.schema_id
left join [OpenMiracle.Cleanup1].sys.dm_db_partition_stats p
    on  p.object_id = t.object_id
    and t.type_desc = 'USER_TABLE'
where t.type_desc not in ('SYSTEM_TABLE', 'INTERNAL_TABLE')
group by s.name, t.name, t.type_desc
